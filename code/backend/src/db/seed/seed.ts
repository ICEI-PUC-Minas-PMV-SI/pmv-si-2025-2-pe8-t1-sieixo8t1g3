...
async function seedTable(tableName: string, tableSchema: any, fileName: string) {
  ...
  if (records.length === 0) {
    ...
    const items = data.map((item: { id: string, name: string, active: boolean }) => ({
      id: item.id,
      name: item.name,
      active: item.active,
      createdBy: ADMIN_ID,
...
async function seed() {
  ...
  await seedTable("competence", schema.competence, "competence.json");
  await seedTable("court", schema.court, "court.json");
  await seedTable("jurisdiction", schema.jurisdiction, "jurisdiction.json");
  await seedTable("proceduralStage", schema.proceduralStage, "proceduralStage.json");
